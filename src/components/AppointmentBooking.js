...
import { StyleSheet, View, Text, TextInput, TouchableOpacity, ScrollView, Picker, Alert, Platform } from 'react-native';
...
const AppointmentBooking = ({ navigation, route }) => {
  ...
  const generateTimeSlots = (doctorAvailability, bookedAppointments) => {
    ...
    const startHour = currentDate.getDate() === selectedDate.getDate() ? Math.max(currentDate.getHours() + 1, 11) : 10;

    for (let i = startHour; i <= 17; i++) {
      if (i === 17 && currentDate.getMinutes() > 30) continue;
      const hour = i < 10 ? '0' + i : i;
      const slot = `${hour}:00`;
      const halfSlot = `${hour}:30`;
